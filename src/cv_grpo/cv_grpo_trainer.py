...
class GRPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(
        ...
        if self.method in ("cv_grpo", "rloo"):
            
            #CV-GRPO
            grouped_rewards = rewards.view(-1, self.num_generations)
            r_bar = grouped_rewards.mean(dim=1, keepdim=True)
            sigma_hat_sq = ((grouped_rewards-r_bar)**2).mean(dim=1, keepdim=True)

            z = (self.num_generations / (self.num_generations-1))*(grouped_rewards -r_bar)

            c = (self.num_generations / (self.num_generations-1))*((grouped_rewards-r_bar)**2-sigma_hat_sq)

            z_centered = z - z.mean(dim=1, keepdim=True)
            c_centered = c - c.mean(dim=1, keepdim=True)
            cov_zc = (z_centered * c_centered).mean(dim=1, keepdim=True)  # [B, 1]
            var_z = z.var(dim=1, unbiased=False, keepdim=True) + 1e-8
            var_c = c.var(dim=1, unbiased=False, keepdim=True) + 1e-8

            lambda_star = cov_zc / var_c  # [B, 1]
            ...
            if self.scale_rewards:                     
                adv_mean_batch = advantages.mean(dim=0, keepdim=True)
                adv_std_batch  = advantages.std(dim=0, unbiased=False, keepdim=True) + 1e-8
                advantages = (advantages - adv_mean_batch) / adv_std_batch
            
            mean_grouped_rewards = rewards.view(-1, self.num_generations).mean(dim=1)
            std_grouped_rewards = rewards.view(-1, self.num_generations).std(dim=1)
            
            # --- 추가: Advantage 분산, 평균, SNR 로그용 ---
            adv_mean = advantages.mean().item()
            adv_std = advantages.std(unbiased=False).item()
            adv_var = adv_std ** 2
            adv_snr = abs(adv_mean) / (adv_std + 1e-8)
            rho2_batch_mean = rho_squared.mean().item()

            residual_var = (var_z.mean() * (1 - rho_squared.mean())).item()
            self._metrics[mode]["residual_var"].append(residual_var)
            self._metrics[mode]["rho2"].append(rho2_batch_mean)
